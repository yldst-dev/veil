...
기존 OCR 솔루션들은 분명 강점이 있었지만, 내가 원하던 문제를 가장 간단하게 해결해주지는 못했습니다.

- ABBYY, Adobe 같은 상용 제품은 정확한 OCR을 제공하지만 가격 부담이 컸습니다.
- PDF를 searchable하게만 만들고 싶은데도, 무거운 PDF 뷰어 기능이나 전체 문서 편집 기능까지 함께 따라오는 경우가 많았습니다.
- 무료 프로그램이나 오픈소스 프로젝트는 변환 시간이 오래 걸리거나 정확도가 아쉬운 경우가 있었고, 환경 설정 자체가 진입장벽이 되기도 했습니다.

그런데 macOS의 Apple Vision OCR은 이미 높은 정확도와 인식률을 보여주고 있었습니다.  
그렇다면 왜 이 기능을 이용해, 스캔 PDF를 원클릭으로 searchable PDF로 바꿔주는 가볍고 실용적인 서드파티 앱은 잘 보이지 않을까 하는 의문이 들었습니다. 그래서 veil을 만들었습니다.

veil은 Apple Vision OCR을 직접 활용해:

- macOS 사용자라면 추가 OCR 비용 없이 사용할 수 있고
- 높은 정확도와 비교적 빠른 처리 속도를 기대할 수 있으며
- 복잡한 설정 없이 스캔 PDF를 바로 searchable PDF로 바꿀 수 있도록 설계했습니다

나는 학생이고, 비싼 OCR 소프트웨어를 계속 구독하거나 구매하기 어려웠습니다.  
그래서 이 도구를 통해, 더 많은 사람이 큰 비용을 들이지 않고도 OCR을 쉽고 빠르게 사용할 수 있게 만들고 싶었습니다.
...
- Release page: [GitHub Releases](https://github.com/yldst-dev/veil/releases)
- DMG: [veil-0.1.2-arm64.dmg](https://github.com/yldst-dev/veil/releases/download/v0.1.2/veil-0.1.2-arm64.dmg)
- ZIP: [veil-0.1.2-arm64-mac.zip](https://github.com/yldst-dev/veil/releases/download/v0.1.2/veil-0.1.2-arm64-mac.zip)
